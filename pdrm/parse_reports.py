...
for line in f:
  ...
  if m:
    wire_len=m.group(1)
    continue
  m=re.search(r'\sLayer\s+li1 :\s+(\d+)',line)
  if m:
    layer_li1=m.group(1)
    continue
  m=re.search(r'\sLayer\s+met1 :\s+(\d+)',line)
  if m:
    layer_met1=m.group(1)
    continue
  m=re.search(r'\sLayer\s+met2 :\s+(\d+)',line)
  if m:
    layer_met2=m.group(1)
    continue
  m=re.search(r'\sLayer\s+met3 :\s+(\d+)',line)
  if m:
    layer_met3=m.group(1)
    continue
  m=re.search(r'\sLayer\s+met4 :\s+(\d+)',line)
  if m:
    layer_met4=m.group(1)
    continue
  m=re.search(r'\sLayer\s+met5 :\s+(\d+)',line)
  if m:
    layer_met5=m.group(1)
    break
f.close()
...
for line in f:
  ...
  if m:
    scenario=m.group(1)
    continue
  m=re.search(r'\s+Cell Internal Power\s+=\s+([0-9.eE+-]+)',line)
  n=re.search(r'\s+Net Switching Power\s+=\s+([0-9.eE+-]+)',line)
  p=re.search(r'Cell Leakage Power\s+=\s+([0-9.eE+-]+)',line)

  if scenario=='mode_norm.fast.RCmin':
    if m:
      power_int=m.group(1)
      continue
    if n:
      ...
      continue
    if p:
      power_leak=p.group(1)
      break
f.close()
# route_opt_power_int_tot
results.write(power_int+',')
# route_opt_power_sw_tot
results.write(power_sw+',')
# route_opt_power_leak_tot
